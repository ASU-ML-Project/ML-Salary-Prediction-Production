...
def predict_salary(req: SalaryRequest):
    ...
    # Track prediction in stats time series
    now_str = datetime.datetime.now().strftime("%H:%M")
    last_volume = STATS_TIME_SERIES[-1]["volume"] if STATS_TIME_SERIES else 0
    new_entry = {
        "time": now_str,
        "volume": last_volume + 1,
        "salary": rounded_prediction,
        "role": req.job_title,
        "education": req.education_level,
        "experience": req.years_experience,
        "age": req.age
    }
    STATS_TIME_SERIES.append(new_entry)

    return {
        "predicted_salary": rounded_prediction,
        "currency": "INR",
        "model_version": "v1.0-2026",
        "prediction_record": new_entry
    }
